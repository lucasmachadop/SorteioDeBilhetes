...
public class Sorteio extends JFrame {
	static String Versao = "0.1";

	static int NumeroFaixas = 100;
	static boolean AferindoPrograma = false;

	// Valores da Extração nº 04957 de 28/03/2015 da Loteria Federal
	...
	public Sorteio() throws ParseException {
		...
		JLabel lblNewLabel = new JLabel("Extração nº:");
		...
		JLabel lblNewLabel_2 = new JLabel("Sorteio nº:");
		...
		lblVersao.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				JOptionPane.showMessageDialog(null, "Versão do Programa de Sorteio: " + Versao + "\r\n\r\n" +
						...
						"Memória Ocupada: " + Runtime.getRuntime().totalMemory() / (1024 * 1024) + " Mbytes\r\n" +
						"Memória Máxima:  " + Runtime.getRuntime().maxMemory() / (1024 * 1024) + " Mbytes\r\n\r\n" +
						"Sistema Operacional: " + System.getProperty("os.name") + "\r\n" +
						"Arquitetura: " + System.getProperty("os.arch") + "\r\nVersão: " +
						System.getProperty("os.version") + "\r\n\r\n" +// "\r\n\r\n"+
						"Este programa é uma adaptação do Programa de " + "\r\n" +
...
//			if ((TipoExtracao.TipoExtracaoEscolhida != TipoExtracao.ValoresTipoExtracao.IMPORTADA) || (ProximaExtracao == 0)) {
			if ((ProximaExtracao == 0)) {
				...
				fc.addChoosableFileFilter(new TextFileFilter());	// Adicionando o filtro para arquivos de extensão txt.
				...
				if (!NomeArquivoSAIDA.substring(NomeArquivoSAIDA.length() - 4).toLowerCase().contains("." + extensao)) {
					// O nome do arquivo não contém a extensão correta.
					NomeArquivoSAIDA += "." + extensao;	// Colocar a extensão no nome do arquivo.
				}
				ArquivosTXT.clear();
			}
